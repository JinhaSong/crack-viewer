...
def save_image_binary(seg_img_path) :
    try :
...
def save_image_binary_thresholding(seg_img_th_path, severity_threshold) :
    try :
...
def save_image_hightlight_region_th(seg_img_path, seg_img_hl_th_path, severity_threshold, region_results, patch_size, image_height, image_width) :
    try :
        seg_img_th = cv2.imread(seg_img_path, -1)
        seg_img_th[seg_img_th <= severity_threshold] = 0
        seg_img_th[seg_img_th > severity_threshold] = 255
        tmp = cv2.cvtColor(seg_img_th, cv2.COLOR_BGR2GRAY)
        _, alpha = cv2.threshold(tmp, 0, 200, cv2.THRESH_BINARY)
        ...
        for region in region_results:
            ...
            for patch in region_area:
                x = patch['x']
                y = patch['y']
                patch = seg_img_th[y:y + patch_size, x:x + patch_size]
                patch[np.where((patch != [0, 0, 0]).all(axis=2))] = [b, g, r]
                seg_img_hl[y:y + patch_size, x:x + patch_size] = patch
        ...
        rgba = [b, g, r, alpha]
        seg_img_hl = cv2.merge(rgba, 4)
        cv2.imwrite(seg_img_hl_th_path, seg_img_hl)
        return True
    except :
        return False